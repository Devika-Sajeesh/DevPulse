...
async def analyze(request: RepoRequest):
    try:
        ...
        # Save to DB - UPDATED CALL SIGNATURE
        report_id = save_report(
            results["repo_url"],
            results["git_sha"], # New Field
            results["radon"],
            results["cloc"],
            results["pylint"],
            results["ai_metrics"], # Updated Field
            results["code_health_score"], # New Field
            results["historical_risk_score"], # New Field
        )
